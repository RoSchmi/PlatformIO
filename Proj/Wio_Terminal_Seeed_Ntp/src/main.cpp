...
// Credentials are in '/include/config_secret.h'
...
void setup() {
    ...
    // Changed by RoSchmi, I take UTC Time
    utcTime = getNTPtime();
    devicetime = utcTime;
 
    lcd_log_line(itoa((int)devicetime, buf, 10));
    ...
    lcd_log_line((char *)"RTC time is:");
    lcd_log_line((char *)(now.timestamp(DateTime::TIMESTAMP_FULL).c_str()));
    ...
    Serial.println(now.timestamp(DateTime::TIMESTAMP_FULL));
    lcd_log_line((char *)"Adjusted RTC UTC-time is:");
    lcd_log_line((char *)(now.timestamp(DateTime::TIMESTAMP_FULL).c_str()));
    ...
    updateDelay.start(30 * 1000); // update time via ntp every 30 sec
...
void loop() {
    ...
    if (updateDelay.justFinished()) { // 12 hour loop
        ...
        devicetime = getNTPtime();
        if (devicetime == 0) {
            lcd_log_line((char *)"Failed to get Internet Time:");
            ...
            Serial.println(now.timestamp(DateTime::TIMESTAMP_FULL));
            lcd_log_line((char *)"Adjusted UTC-Time is:");
            lcd_log_line((char *)(now.timestamp(DateTime::TIMESTAMP_FULL).c_str()));
        }
    }
}
