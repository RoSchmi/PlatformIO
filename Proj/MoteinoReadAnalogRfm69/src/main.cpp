...
// To include libraries, eventually this must be added to platformio.ini;
// lib_deps = TimeOut
...
//#define FEATHER_M0              //comment out if not Feather_M0 board
//#define DebugPrint                //comment out if certain messages are not wanted
...
#ifdef FEATHER_M0
  #define LED           13  // Feather M0 has LED on 13
#endif
...
#ifdef FEATHER_M0
int CollectorAnalogPin = 1;
int StorageAnalogPin = 1;
int WaterAnalogPin = 1;
#endif
...
int valCollector = 0;  // variable to store the voltage value read 
int valStorage = 0;
int valWater = 0; 
...
//****************** End Definitions for Rfm69 Module **************************
...
const uint32_t InitialReleaseTimespan_CollectorTemp_Ms = 1000;   // First 'CollectorTemp' read after this time
const uint32_t InitialReleaseTimespan_StorageTemp_Ms = 1000;     // First 'StorageTemp' read after this time
const uint32_t InitialReleaseTimespan_WaterTemp_Ms = 1000;      // First 'WaterTemp' read after this time

const uint32_t FollowReleaseTimespan_CollectorTemp_Ms = 5000;   // Following 'CollectorTemp' reads after this time
const uint32_t FollowReleaseTimespan_StorageTemp_Ms = 5000;     // Following 'StorageTemp' reads after this time
...
yield();   //Passes control to other tasks when called.
  ...
  #ifdef FEATHER_M0
    Serial.println("analogReadResolution(12) has to be not commented out");
    //analogReadResolution(12);
  #endif

  // Pseudo random
  packetnum = (int)analogRead(CollectorAnalogPin) & 0x000000FF;
#ifdef DebugPrint
  Serial.print("Packetnumber: "); Serial.println(packetnum);
#endif

  
  // Hard Reset the RFM module only for Feather M0
#ifdef FEATHER_M0
...
#ifndef FEATHER_M0
  _slaveSelectPin = RFM69_CS;    // Moteino: D10
  _interruptPin = RFM69_IRQ;     // Moteino: D2
  _interruptNum = RFM69_IRQN;    // Moteino: Pin 2 is IRQ 0!
  #endif
  ...
  setPowerLevel(_powerLevel); // power output ranges from 0 (5dBm) to 31 (20dBm)
...
void loop() {
  ...
  if (readIntervalCounter > readIntervalLengthSec)
  {
        ...
        ActCollectorTemp = countsToValueFloat(valCollector, CollectorSensorParams);
        ActStorageTemp = countsToValueFloat(valStorage, StorageSensorParams);
        ActWaterTemp = countsToValueFloat(valWater, WaterSensorParams);


      #ifdef SERIAL_EN 
        
        Serial.print(F("Read CollectorValue = "));
        Serial.print(valCollector);
        Serial.print(twoSpaces);
        Serial.println(ActCollectorTemp, 1);
           
        Serial.print(F("Read StorageValue = "));
        Serial.print(valStorage);
        Serial.print(twoSpaces);
        Serial.println(ActStorageTemp, 1);

       
        Serial.print(F("Read WaterValue = "));
        Serial.print(valWater);
        Serial.print(twoSpaces);
        Serial.println(ActWaterTemp, 1);
        Serial.println("");
      #endif
        ...
        display.print("Coll: ");
        display.print(valCollector);
        display.print(twoSpaces);
        display.println(ActCollectorTemp, 1);
              
        display.setCursor(0,10);
        display.print("Stor: ");
        display.print(valStorage);
        display.print(twoSpaces);
        display.println(ActStorageTemp, 1);

        display.setCursor(0,20);
        display.print("Water: ");
        display.print(valWater);
        display.print(twoSpaces);
        display.print(ActWaterTemp, 1);
        display.display();
    #endif

      dataContainerTemp.SetNewValues(millis(), ActCollectorTemp, ActStorageTemp, ActWaterTemp); 
    ...
    // Read from the console to test the temperature calculation from corresponding voltage
    recvWithEndMarker();
    if (newData == true)
    {
    ...
    if (dataContainerTemp.hasToBeSent())
      {
